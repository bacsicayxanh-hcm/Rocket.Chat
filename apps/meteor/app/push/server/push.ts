...
class PushClass {
	...
	private async sendNotificationGateway(
		...
		for (const gateway of this.options.gateways) {
			logger.debug('send to token', app.token);
			const appNameIsBSCX = app.appName?.includes('bacsicayxanh');
			const gatewayIsBSCX = gateway.includes('bacsicayxanh');
			const isRun = !!(appNameIsBSCX === gatewayIsBSCX && appNameIsBSCX !== undefined);
